...
| `0x4E5560` / `0x4E5680` | Resolves the active/current view from the controller's ordered list at `controller+0x298`; `0x4E5680` is a thin wrapper. The returned object's type name is an inline narrow string beginning at `view+0x0`, **not a pointer**. See `0x4E57E0`, which passes that same object address directly to `strncmp`. |
| `0x4E57E0` | Scans controller views by inline type-name prefix and overwrites its output on every match; returns the last matching view, which need not equal the active one. Its `strncmp` accepts both `vg_listview_explorer` and suffixed `vg_listview_explorer##1` objects. |
| `0x43A0C0` | Searches a distinct controller list at `controller+0x34`; its entry layout dereferences `view+0` to reach a type-name pointer. Do not transfer that layout to the `controller+0x298` views used by `0x4E5560`/`0x4E57E0`. |
